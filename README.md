...
1. Push the repository to GitHub and enable Pages for the main branch.
...
Each pytest run overwrites `pytest-results.md` in the repository root with a short Markdown summary (counts, duration, exit status). Commit it alongside code changes so readers can see the latest test outcome.
